...
const NotificationsPage = () => {
  ...
  const handleAction = (action) => {
    ...
    if (action.type === "link" && action.destination) {
      // Keep the destination path as is
  ...
  const notificationPriorities = useMemo(() => {
    const priorities = new Set(notifications.map(notification => notification.priority));
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {loading ? (
          ...
          <div className="bg-red-50 text-red-600 p-6 rounded-xl shadow-md mb-6 border border-red-100">
            <p className="font-semibold text-lg mb-2">Error loading notifications</p>
            <p>{error}</p>
          </div>
                  ...
                  <div className="flex items-start">
                    ...
                    <div className="ml-2 flex-1">
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                        <h3 className={`text-base font-medium ${!notification.read ? "text-blue-800" : "text-gray-800"}`}>
                          {notification.title}
                        </h3>
                      ...
                      <p className="mt-1.5 text-sm text-gray-600">{notification.message}</p>
                          ...
                          {Array.isArray(notification.details) ? (
                            <ul className="space-y-2">
                              {notification.details.map((detail, index) => (
                                <li key={index} className="flex flex-col sm:flex-row sm:justify-between border-b border-gray-100 pb-2 last:border-0 last:pb-0">
                                  <span className="font-medium">{detail.productName}</span>
                                  <span className="mt-1 sm:mt-0">
                                    Stock: <span className={detail.currentStock < detail.reorderPoint ? "text-red-600 font-bold" : ""}>
                                      {detail.currentStock}
                                    </span> 
                                    {detail.reorderPoint && ` (Reorder at: ${detail.reorderPoint})`}
                                  </span>
                                </li>
                              ))}
                            </ul>
