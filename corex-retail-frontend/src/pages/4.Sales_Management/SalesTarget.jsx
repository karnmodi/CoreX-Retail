...
export default function SalesTargetsPage() {
      ...
      <div className="flex flex-col">
        ...
        <div className="p-4 md:p-6 space-y-4">
          <div className="text-red-500 p-4 border border-red-300 rounded bg-red-50">
            <h2 className="text-lg font-medium mb-2">Error loading targets</h2>
            <p>{error}</p>
  ...
  const getTargetSummary = (type) => {
    if (!salesTargets || !salesTargets.summary) return { target: 0, achieved: 0, percentage: 0 };
    ...
    let targetKey = '';
    if (type === 'monthly') {
      targetKey = `monthly-${currentYear}-${String(currentMonth).padStart(2, '0')}`;
    ...
    return salesTargets.summary[targetKey] || { target: 0, achieved: 0, percentage: 0 };
  };
    ...
    <div className="flex flex-col">
      ...
      <div className="p-4 md:p-6 space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {[monthlyTarget, quarterlyTarget, yearlyTarget].map((target, index) => (
            <Card key={index}>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium">
                  {index === 0 ? 'Monthly Target' : index === 1 ? 'Quarterly Target' : 'Annual Target'}
                </CardTitle>
              </CardHeader>
              <CardContent>
                ...
                <div className="mt-2 h-2 w-full rounded-full bg-muted">
                  <div
                    className={`h-full rounded-full ${target.percentage >= 75
                      ? 'bg-green-500'
                      : target.percentage >= 50
                        ? 'bg-yellow-500'
                        : 'bg-red-500'}`}
                    style={{ width: `${Math.min(target.percentage || 0, 100)}%` }}
                  ></div>
                </div>
                <p className="mt-1 text-xs text-muted-foreground">
                  {target.percentage?.toFixed(1) || '0'}% achieved (£{target.achieved?.toLocaleString() || '0'})
                </p>
              </CardContent>
            </Card>
          ))}
